...
def time_dec(func):
    ...
    def wrapper(*arg, **kwargs):
        t = time.time()
        res = func(*arg, **kwargs)
        logging.debug('DURATION: <%s> : ' % func.__name__ + str(time.time()-t))
        return res
    return wrapper
...
def tiles_already_processed(TILESTORE_PATH):
    ...
    logging.debug("tiles_already_processed")
    ...
    for existing_tile in existing_tiles:
        with xr.open_dataset(existing_tile) as ds:
            ...
            if source is not None:
                ...
                logging.warn('Source attr not set in file %s.' % existing_tile)
...
def match_watermask_shpfile(glob_string):
    ...
    logging.debug("match_watermask_shpfile")
    ...
    if len(glob.glob(glob_string)) == 0:
        ...
        logging.error("Too many shape files.")
        exit()
...
def get_tile_summary(ds, cutoff=0):
    ...
    logging.debug("get_tile_summary")
...
def tile_files_compatible(files):
    ...
    logging.debug("tile_files_compatible")
...
def create_stats_table(df, var):
    ...
    logging.debug("create_stats_table")
...
def convert_dem_files(cfg, lf_ele_levels):
    ...
    logging.debug("convert_dem_files")
    ...
    if cfg.SRTMSTORE_PATH is not None:
        ...
        for dem_file in dem_files:
            ...
            if process_tiles:
                logging.info('processing: %s (%s)' % (dem_file, datetime.datetime.now()))
                ...
                for i, tile in enumerate(tiles):
                    ...
                    if tile != None and has_significant_land(tile):
                        logging.debug("Valid tile %d in file %s." % (i+1, dem_file))
                        ...
                        logging.debug("Empty tile %d in file %s ignored." % (i+1, dem_file))
...
def compute_statistics(cfg):
    ...
    logging.debug("compute_statistics")

    available_tiles = glob.glob(os.path.join(cfg.TILESTORE_PATH, '*.nc'))
    logging.debug('Number of tiles found: %d' % len(available_tiles))
    if len(available_tiles) == 0:
        logging.error('No processed tiles available in directory "%s"' % cfg.TILESTORE_PATH)
        exit()
    tiles = sorted(available_tiles)

    if not tile_files_compatible(tiles):
        logging.error('Tile files in %s are not compatible.' % cfg.TILESTORE_PATH)
        exit()

    tiles_stats = []
    for tile in tiles:
        logging.debug('Computing statistics for tile %s' % tile)
...
def spatialclip_df(df, extent):
    ...
    if any(e is None for e in extent):
        logging.warn("SpatialClip: extent passed is None.")
    lon1, lat1, lon2, lat2 = extent

    if ('lon' not in df.columns) or ('lat' not in df.columns):
        logging.warn("SpatialClip: lat/ lon column missing in df.")
...
def build_site_netcdf(soilref, elevref, extent=None):
    ...
    logging.debug("build_site_netcdf")

    logging.debug("soilref: %s", soilref)
    logging.debug("elevref: %s", elevref)
    ...
    # no soil data but elevation: gap-fill with neighbors
    missing = np.where((smask == 1) & (emask == 0), 1, 0)
    ix, jx = np.where(missing == 1)

    if len(ix) > 0:
        logging.debug('Cells with elevation but no soil data [BEFORE GF: %d].' % len(ix))
        ...
        for i, j in zip(ix, jx):
            for v in soil_vars:
                if np.isfinite(ds_soil[v][i, j-1]):
                    ...
                    logging.warn('neighbours have nodata !')
        ...
        ix, jx = np.where(missing == 1)
        logging.debug('Cells with elevation but no soil data [AFTER GF:  %d].' % len(ix))
    ...
    for v in soil_vars:
        ...
        da.name = varSoil[v][0]
        logging.debug("da.name: %s", da.name)
...
def build_landform_netcdf(lf_full_set, df_dict, cfg, elevation_levels, refnc=None):
    ...
    logging.debug('df_extent: %s' % str(df_extent))
    logging.debug('contains: %s' % str(refnc.geo.contains(df_extent)))
...
def build_compressed(ds):
    ...
    if 'lfcnt' in ds.data_vars:
        ...
        logging.error("Not a valid xr.Dataset (landforms or site only).")
...
def main(cfg):
    ...
    logging.info("Converting DEM files and computing landform stats")

    # define the final landform classes (now with elevation brackets)
    # TODO: Fix hard coded values: 200, 6000
