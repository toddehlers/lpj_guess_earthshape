...
// aspect 
typedef enum {
    NORTH = 0, EAST = 90, SOUTH = 180, WEST = 270, NOASPECT = -1
} aspect_type;

typedef enum {
    RIDGE = 1, UPSLOPE = 2, MIDSLOPE = 3, FLAT = 4, DOWNSLOPE = 5, VALLEY = 6
} slopepos_type;
...
public:
	...
	/// mean rad for the last 31 days (W m-2)
	double mrad;
	...
	/// daily rad for the last 31 days (W m-2)
	Historic<double, 31> drad_31;
    ...
    /// cw SubPixel clone climate
    void clone(Gridcell& gc, Climate& local_climate);
...
// cw SubPixel
// Storage class of landform terrain characteristics for subpixel mode
struct Landform : public Serializable {
    /// landform identifier: e.g. 1241 [elevation slice.:1-n, 400m atm][hillpos:1-6][aspect:0-4]
    int id;
    /// fraction of gridcell covered by this stand/ landform (0...1)
    double fraction;
    /// local elevation of this landform
    double elevation;
    /// avg. aspect of this landform (0: North, 90: East, 180: South, 270: West)
    aspect_type aspect;
    double avg_aspect;
    /// avg. slope of this landform
    double slope;
    /// avg. aspect corrected slope of this landform
    double asp_slope;
    /// avg. soildepth of this landform
    double soildepth;
    /// depth modifier for lower layer
    double soildepth_lower_mod;
    /// indicator for deepwater store in this landform
    bool has_deepwater;
...
// Storage class of gridcell site and soil characteristics for subpixel mode
...
public:
    ...
    // cw additions
    int aphen_max;              // max phenology days forseasonal pfts
    double nrelocfrac;          // fraction of N relocated (retained)
    bool has_deepwater_access;  // deepwater access flag
    double fwuptake_deep;       // fwuptake from deep water
...
public:
	...
	Pft() {
        ...
        // cw flexible aphen_max, nrelocfrac, deepwater access
        aphen_max = 210;
        nrelocfrac = 0.5;
        has_deepwater_access = false;
        fwuptake_deep = 0.0;
	...
	void initsla() {
		...
		if (!(phenology == CROPGREEN && ifnlim)) {
            ...
            // cw SWITCH TO GLOBAL LEAFPHYSIOGNOMY VALUES
			if( ifuniversalsla ){
                sla = 0.2 * pow(10.0, 2.44 - 0.43 * log10(12.0 * leaflong));
            } else {
...
public:
	...
	double mlai_max[12];
	/// cw SubPixel mothly fractional projected cover
	double mfpc[12];
...
public:
    ...
    double dwcont_deep[Date::MAX_YEAR_LENGTH];
    
    /// cw daily nfix
    Historic<double, 10> daet;
...
public:
	...
	void initdrivers() {
        ...
        for (unsigned int d=0; d<daet.CAPACITY; d++){
            daet.add(0.0);
        }
        ...
        std::fill_n(dwcont_deep, Date::MAX_YEAR_LENGTH, 0.0);
...
public:
    ...
    /// fractional uptake from deepwater today
    double fwuptake_deep;
...
public:
    ...
    // cw SubPixel local climate (based on gridcell.climate, but altered for landform diff)
    Climate local_climate;

    // cw SubPixel landform struct (TODO: MAKE SURE TO SERIALIZE THIS, TOO!)
    Landform landform;
    ...
    // cw SubPixel alternative constructor for landform mode
    Stand(int i, Gridcell* gc, Soiltype& st, landcovertype landcoverX, Landform landformX, int np_patch = 0);
...
private:
    ...
    // cw SubPixel
    // init function to allow clean second stand constructor
    void init(int np_patch, Landform landformX);
};
...
public:
    ...
    // cw SubPixel
    /// Site information for this grid cell
    Site get_site() const;

    // cw SubPixel
    /// Set site info for this gridcell
    void set_site(Site siteX);
    ...
    // cw SubPixel
    Stand& create_stand(landcovertype lc, Landform lf, int no_patch = 0);
