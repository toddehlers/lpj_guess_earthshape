...
// cw a pft specific parameter
// double nrelocfrac;
bool ifdeepwater;
bool ifuniversalsla;    // use universal SLA calculation from Raich 1992 instead of breadleaf/ needleleaf specific ones
...
// cw baseflow frac (default: 0.5)
double BASEFLOW_FRAC;
...
// cw Subpixel
bool run_landform;
bool sp_modifysoildepth;
int spoutput_startyear;
int spoutput_step;
...
void plib_declarations(int id,xtring setname) {
	...
	case BLOCK_GLOBAL:
        ...
        // cw now a pft specific parameter
		...
		declareitem("ifnfixdaily",&ifnfixdaily,1,CB_NONE,
			"Whether to use improved daily nfix calculation");

        declareitem("BASEFLOW_FRAC",&BASEFLOW_FRAC,0,1,1,CB_NONE,
                    "Fraction of baseflow percolation going to drain");
		...
		// cw SubPixel
		declareitem("run_landform",&run_landform,1,CB_NONE,"Landform version");
		declareitem("sp_modifysoildepth",&sp_modifysoildepth,1,CB_NONE,"Modify soil depth according to topography position");
        declareitem("ifdeepwater",&ifdeepwater,1,CB_NONE,"Activate deep water access functionality");
        declareitem("ifuniversalsla",&ifuniversalsla,1,CB_NONE,"Use universal SLA calculation from Raich 1992");

		declareitem("spoutput_startyear",&spoutput_startyear,-1,9999,1,CB_NONE,"Start year of per stand/patch output (-1 no restriction)");
		declareitem("spoutput_step",&spoutput_step,1,100,1,CB_NONE,"Step width for stand/patch output");
        ...
        declareitem("landform_state_path", &landform_state_path, 300, CB_NONE, "State files directory (for saving state files in run_landform mode)");
	...
	case BLOCK_PFT:
        ...
        // cw flexible aphen_max, nrelocfrac now per pdt, optional deepwater access flag
        declareitem("aphen_max",&ppft->aphen_max,0,365,1,CB_NONE,
            "Maximum phenology for summergreen (days)");
        declareitem("nrelocfrac",&ppft->nrelocfrac,0.0,0.99,1,CB_NONE,
            "Fractional nitrogen relocation from shed leaves & roots");
        declareitem("has_deepwater_access",&ppft->has_deepwater_access,1,CB_NONE,
            "whether (1) or not (0) this pft has access to deepwater store");
...
void plib_callback(int callback) {
	...
	case CB_CHECKGLOBAL:
		...
		// cw SubPixel
		if (!itemparsed("run_landform")) badins("run_landform");
		if(run_landform) {
			if (!itemparsed("spoutput_startyear")) spoutput_startyear = -1;
			if (!itemparsed("spoutput_step")) spoutput_step = 1;
		}
		...
		// cw SubPixel allow read and save
		if(run_landform){
			dprintf("READ and DUMP state\n");
		} else {
        ...
        if (landform_state_path == "" && run_landform) {
            badins("landform_state_path");
        }
	...
	case CB_CHECKPFT:
		if (!checked_pft[ppft->name]) {
            ...
            // cw flexible aphen_max
            if (!itemparsed("aphen_max")) badins("aphen_max");
