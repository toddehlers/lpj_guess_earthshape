...
void hydrology_lpjf(Patch& patch, Climate& climate, double rain_melt, double perc_base,
	...
	// this is now an optional INS file parameter
    ...
    double aet_deep(0.0);        // AET from deep water
	double perc_frac(0.0);
	...
	while (vegetation.isobj) {
        ...
        // deepwater aet
        aet = patch.pft[indiv.pft.id].fwuptake_deep * indiv.aet;
        aet_deep += aet;
        aet_total += aet;
	...
	// cw SubPixel flexible soildepth
    double modUL = 1.0;
    double modLL = 1.0;
    double mDL   = 2.0; // extra multiplier for the deep layer height
	...
	for (s=1; s<NSOILLAYER; s++) {
		...
		// modLL (lower layer modifyer)
		// modUL (upper layer modifyer) - no effect in 2 layer runs
		if (run_landform && sp_modifysoildepth){
			modLL = patch.stand.landform.soildepth_lower_mod;
			if (s>1){
				modUL=patch.stand.landform.soildepth_lower_mod;
			}
		}
		
		perc_frac = min(perc/(awc[s-1]*modUL), wcont[s-1]);

		wcont[s-1] -= perc_frac;
		wcont[s] += perc_frac * awc[s-1]*modUL / (awc[s]*modLL);
		if (wcont[s] > 1.0) {
			runoff_drain += (wcont[s]-1.0) * (awc[s]*modLL);
			wcont[s] = 1.0;
		}
		...
		wcont[s] -= aet_layer[s] / (awc[s]*modLL);
	...
	if (percolate) {
        ...
        if (patch.stand.landform.has_deepwater){
            // partition baseflow (10% - 25% normal baseflow, 75% deep baseflow (potentially into storage)
            double perc_baseflow0 = perc_baseflow * 0.1;
            perc_deepflow = perc_baseflow - perc_baseflow0;
            perc_baseflow = perc_baseflow0;

            // wcont_deep
            wcont[NSOILLAYER-1] -= perc_deepflow / (awc[NSOILLAYER-1]*modLL);
            patch.soil.wcont_deep += perc_deepflow / (awc[NSOILLAYER-1]*modLL*mDL);
            if (patch.soil.wcont_deep > 1.0) {
                runoff_baseflow += (patch.soil.wcont_deep-1.0) * (awc[NSOILLAYER-1]*modLL*mDL);
                patch.soil.wcont_deep = 1.0;
            }
        }
        
		// Deduct from water content of bottom soil layer
		perc_frac = min(perc_baseflow/(awc[NSOILLAYER-1]*modLL), wcont[NSOILLAYER-1]);
		wcont[NSOILLAYER-1] -= perc_frac;
		runoff_baseflow += perc_frac * (awc[NSOILLAYER-1]*modLL);
	}

    // since the deepwater storage layer has the same awc properties as the second default layer
    // we simply use the data from this layer
    
    // do a small (continuous) drain from the deep water layer (should maybe depend on texture?)
    // daily drain amount from deepwater storage layer (1mm)
    double deepleak_amount(0.01);  // in mm day-1
    double daily_deepleak_frac(0.0);
    
    if((patch.soil.wcont_deep * awc[NSOILLAYER-1] * modLL * mDL) < deepleak_amount){
        deepleak_amount = patch.soil.wcont_deep * (awc[NSOILLAYER-1]*modLL*mDL);
    }
    daily_deepleak_frac = min(deepleak_amount / (awc[NSOILLAYER-1]*modLL*mDL), patch.soil.wcont_deep);
    
    if (daily_deepleak_frac < 0){
        fail("WHOA!\n");
    }
    patch.soil.wcont_deep = max(patch.soil.wcont_deep, 0.0);
    
    
    patch.soil.wcont_deep -= daily_deepleak_frac;
    runoff_deepwater += daily_deepleak_frac * (awc[NSOILLAYER-1]*modLL*mDL);

	// save percolation from system (needed in leaching())
	dperc = runoff_baseflow + runoff_drain + runoff_deepwater;

	runoff = runoff_surf + runoff_drain + runoff_baseflow + runoff_deepwater;

	patch.asurfrunoff += runoff_surf;
	patch.adrainrunoff += runoff_drain;
	patch.abaserunoff += runoff_baseflow;
    patch.adeeprunoff += runoff_deepwater;
