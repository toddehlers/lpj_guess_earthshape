...
public:
	...
	// cw SubPixel Gets the year offset for paelo sims
	int get_paleo_offset() const {return paleo_refyear; };
...
private:
	...
	// cw SubPixel the offset of paleo years for get_calendar_years functionality
	int paleo_refyear;
...
// cw SubPixel
/// Class for reading data from a NetCDF file (without time dimension)
/** Expected to be used for dealing with one gridcell at a time, so the NetCDF
 *  file should also be stored in that order to get acceptable performance.
 *
 *  Limitations on top of CF:
 *
 *   * Main variable must be numeric NetCDF classic data type.
 *   
 *   * This class is for non-time related data (use the regular GridcellOrderedVariable)
 *     if you need the time dimension
 *
 *   * 2D space dimensions are understood by the class. The main variable
 *     may also have an additional extra dimension (for instance for landforms), which
 *     this class simply treats as 'the extra dimension' if available.
 */
class GridcellOrderedVariableNoTime {
public:
     ...
     *  \param variable The name of the variable to read from
     */
    GridcellOrderedVariableNoTime(const char* filename, const char* variable);
    ...
    /// Gets variable's value for currently loaded location
    /** Use this function to retrieve values in a 2 dimensional data set,
     *  where each (lat,lon) tuple corresponds to a single scalar value.
     *
     *  \see get_values for data sets with an extra dimension
     *
     *  If this function is used with a data set with an extra dimension,
    ...
    /// Gets all values for currently loaded location in a certain timestep
    /** Use this function to retrieve values in a 3 dimensional data set,
     *  for instance when landform is a third dimension.
     *
     *  The values vector will be resized if necessary.
     */
    void get_values(std::vector<double>& values) const;

    void get_values(std::vector<double>& values, std::vector<bool>& missing) const;

    // f_js_20171018 
    void get_landforms(std::vector<int>& lf_ids) const;
    void get_lgt_elevation_step(int& lgt_elevation_step) const;
    void get_lgt_classification(std::string& lgt_classification) const;
    ...
    /// Checks if another GridcellOrderedVariable has the same spatial domain as this one
    bool same_spatial_domain(const GridcellOrderedVariableNoTime& other) const;
...
private:
    ...
    // cw SubPixel
    // handle to the actual landform ids (currently composed in code, should be coming from file)
    int ncid_lfid;
