 ...
 */
double lignin_to_n_ratio(double cmass_litter, double nmass_litter, double LIGCFRAC, double cton_avr, double nrelocfrac) {
...
void transfer_litter(Patch& patch) {
	...
	while (patch.pft.isobj) {
		...
		if (drop_leaf_root_litter) {
			...
			double leaf_lton = lignin_to_n_ratio(pft.litter_leaf, pft.nmass_litter_leaf, LIGCFRAC_LEAF, pft.pft.cton_leaf_avr, pft.pft.nrelocfrac);
			...
			double root_lton = lignin_to_n_ratio(pft.litter_root, pft.nmass_litter_root, LIGCFRAC_ROOT, pft.pft.cton_root_avr, pft.pft.nrelocfrac);
...
void leaching(Soil& soil) {
	...
	if (!negligible(soil.dperc)) {
        ...
        if (run_landform && sp_modifysoildepth){
            minleachfrac = soil.dperc / (soil.dperc + soil.soiltype.awc[0] * soil.wcont[0] +
                                        (soil.soiltype.awc[1] * soil.patch.stand.landform.soildepth_lower_mod) * soil.wcont[1]);
        } else {
...
void soilnadd(Patch& patch) {
	...
	if (soil.nmass_avail < NMASS_SAT) {
		...
		if (ifnfixdaily) {
			...
			daet += patch.devap; 
			soil.daet.add(daet);
			aet_mean = soil.daet.mean();

			const double daily_nfix = max((nfix_a * aet_mean * date.year_length() * cmtomm + nfix_b) * hatom2, 0.0)/365.0;
