...
void establishment_guess(Stand& stand,Patch& patch) {
  ...
  // f_js_20170118
  double est_max_total = 0.0;

  
  // f_js_20170504
  std::vector<double> est_max_all;
  std::vector<int> count_est_max;
  std::set<double> unique_est_max;
    ...
    // f_js_20170118+ PalEON_Species modified to include est_max_total
		if (establish(patch, stand.get_climate(), pft) && pft.lifeform == TREE && standpft.active && (est_this_year || force_planting)) {
			nwoodypfts_estab++;
      //est_max_total += pft.est_max;
      est_max_all.push_back(pft.est_max); // f_js_20170504
    }
    // f_js_20170118-
		pftlist.nextobj();
	}
  ...
  //f_js_20170504+
  std::stable_sort(est_max_all.begin(), est_max_all.end());
  est_max_total = std::accumulate(est_max_all.begin(), est_max_all.end(), 0.0); // replaces "est_max_total += pft.est_max;" above
  unique_est_max = std::set<double>(est_max_all.begin(), est_max_all.end());
  for (std::set<double>::iterator it=unique_est_max.begin(); it!=unique_est_max.end(); ++it)
    count_est_max.push_back(std::count(est_max_all.begin(), est_max_all.end(), *it));
  /*
    if (date.year==5) {
    dprintf("JS_DEBUG %04i: %4.2f %i\n", date.get_calendar_year(), est_max_total, unique_est_max.size());
    for (int i=0; i<est_max_all.size(); i++)
      dprintf(" %4.2f", est_max_all.at(i));
    dprintf("\n");
    }
  */
  //f_js_20170504-
	...
	while (pftlist.isobj) {
		...
		if (stand.pft[pft.id].active) {
          ...
          //f_js_20170504+
          int unique_est_max_id = std::distance(unique_est_max.begin(), unique_est_max.find(pft.est_max));
          est /= (double)count_est_max.at(unique_est_max_id);

          //if (est_max_total > 0.15)
          //  est *= 0.15/est_max_total;
          //f_js_20170504-

          // f_js_20170118+ PalEON_Species modified to use est_max_total and a maximum of 1500 trees/hs; see above
...
void mortality_guess(Stand& stand, Patch& patch, const Climate& climate, double fireprob) {
	...
	const double KMORTGREFF=0.1; // f_js_20170502 due to to low LAI; as in Hickler et al. (2012)
