...
bool get_paleo_offset_years(const char* calendar, CalendarType calendar_type, int& refyear){
	if (calendar_type == PALEO) {
		std::string str(calendar);
		
		// split at 
		std::string str_BP1("B.P.");
		std::string str_BP2("BP");
		
		std::string str_kyr("kyr");
		std::string str_yr("yr");
		
		// determine year unit
		if (str.find(str_kyr) != std::string::npos){
			// kyr calendar
			std::string str_refyear = str.substr(0, str.find(str_kyr));
			refyear = atoi(str_refyear.c_str()) * -1000 + 1950 -1;

		} else if (str.find(str_yr) != std::string::npos){
			// yr calendar
			std::string str_refyear = str.substr(0, str.find(str_kyr));
			refyear = atoi(str_refyear.c_str()) * -1 + 1950 -1;
		} else {
			throw CFError(std::string("Unsupported time unit for Paleo calendar (valid: 22000 yr B.P., 22 kyr B.P.)"));
		}
	}
	else {
...
CalendarType parse_calendar(const char* calendar) {
	...
	else if ((str.find(str_BP1) != std::string::npos) || (str.find(str_BP2) != std::string::npos)) {
		return PALEO;
	}
	else {
		throw CFError(std::string("Unsupported calendar type: ") + calendar);
...
int days_in_month(int year, int month, CalendarType calendar_type) {
	...
	if (calendar_type == NO_LEAP || calendar_type == PALEO) {
...
DateTime::DateTime()
	:year(0), month(1), day(1), seconds(0), paleo_offset(0) {
}
...
bool DateTime::operator==(const DateTime& other) const {
	return year == other.year &&
		...
		paleo_offset == other.paleo_offset;
}
