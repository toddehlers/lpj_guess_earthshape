...
// cw flexible aphen_max: this is now replace with a pft parameter aphen_max (that is only relevant for summergreen pfts)
...
void leaf_phenology_pft(Pft& pft, Climate& climate, double wscal, double aphen,
	...
	if (summergreen) {
		...
		if (pft.lifeform == TREE) {
            ...
            if (climate.gdd5 > pft.gdd0[climate.chilldays] && aphen < pft.aphen_max)
...
void turnover(double turnover_leaf, double turnover_root, double turnover_sap,
	...
	double& longterm_nstore, double &max_n_storage, double nrelocfrac,
	bool alive) {
	...
	double actual_nrelocfrac = calc_nrelocfrac(lifeform, turnover_leaf, nmass_leaf, turnover_root, nmass_root,
	                                           turnover_sap, nmass_sap, max_n_storage, longterm_nstore, nrelocfrac);
		...
		if (negligible(indiv.densindiv))
			...
			if (bminc >= 0 && indiv.pft.phenology == RAINGREEN) {
				...
				if (indiv.alive) {
					...
					patch.pft[indiv.pft.id].nmass_litter_leaf += raingreen_ndemand * (1.0 - indiv.pft.nrelocfrac);
					indiv.nstore_longterm += raingreen_ndemand * indiv.pft.nrelocfrac;
			...
			if (!killed) {
				...
				if(!indiv.has_daily_turnover()) {
					...
					turnover(indiv.pft.turnover_leaf, indiv.pft.turnover_root,
                        ...
                        indiv.pft.nrelocfrac,   // cw now a pft parameter
						indiv.alive);
				}
				...
				if (indiv.pft.lifeform == TREE) {
                    ...
                    // cw nrelocfrac now a pft parameter
					if (cmass_sap_inc < 0.0 && indiv.nmass_sap >= -nmass_sap_inc){
						indiv.nmass_sap += nmass_sap_inc;
						indiv.nmass_heart -= nmass_sap_inc * indiv.pft.nrelocfrac;
						indiv.nstore_longterm -= nmass_sap_inc * (1.0 - indiv.pft.nrelocfrac);
					...
					// Leaf
					patch.pft[indiv.pft.id].nmass_litter_leaf += leaf_inc * (1.0 - indiv.pft.nrelocfrac);
					indiv.nstore_longterm += leaf_inc * indiv.pft.nrelocfrac;

					// Root
					patch.pft[indiv.pft.id].nmass_litter_root += root_inc * (1.0 - indiv.pft.nrelocfrac);
					indiv.nstore_longterm += root_inc * indiv.pft.nrelocfrac;
					...
					if(indiv.pft.phenology != CROPGREEN && !(indiv.has_daily_turnover() && indiv.continous_grass())) {
						...
						if (indiv.nmass_leaf > 0.0){
							patch.pft[indiv.pft.id].nmass_litter_leaf += litter_leaf_inc * indiv.densindiv /
								cton_leaf_bg * (1.0 - indiv.pft.nrelocfrac);
							indiv.nstore_longterm += litter_leaf_inc * indiv.densindiv / cton_leaf_bg * indiv.pft.nrelocfrac;
							// Subtracting litter nitrogen from individuals
						...
						if (indiv.nmass_root > 0.0){
							patch.pft[indiv.pft.id].nmass_litter_root += litter_root_inc * indiv.densindiv /
								cton_root_bg * (1.0 - indiv.pft.nrelocfrac);
							indiv.nstore_longterm += litter_root_inc / cton_root_bg * indiv.pft.nrelocfrac;
				...
				if (!killed && indiv.pft.phenology != CROPGREEN && !(indiv.has_daily_turnover() && indiv.continous_grass())) {
					...
					double retransn_nextyear = indiv.cmass_leaf * indiv.pft.turnover_leaf / cton_leaf_bg * indiv.pft.nrelocfrac +
						indiv.cmass_root * indiv.pft.turnover_root / cton_root_bg * indiv.pft.nrelocfrac;

					if (indiv.pft.lifeform == TREE)
						retransn_nextyear += indiv.cmass_sap * indiv.pft.turnover_sap / cton_sap_bg * indiv.pft.nrelocfrac;
