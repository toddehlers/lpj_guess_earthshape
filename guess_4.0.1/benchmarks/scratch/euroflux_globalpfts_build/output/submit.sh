...
# /esd/esd01/data/willi/lpj_git/lpj-guess/benchmarks/scratch/euroflux_globalpfts_build/output/guess
# as a parallel job on a multicore system.
#
# Created automatically on Fri Mar 23 16:57:15 CET 2018
# 
# Usage: 
#
...
do
  ...
  cd run$a
  ( if ! /esd/esd01/data/willi/lpj_git/lpj-guess/benchmarks/scratch/euroflux_globalpfts_build/output/guess -input $INPUT_MODULE $INSFILE 1>> ../stdout.txt 2>> ../stderr.txt ; then
      echo -n $a" " >> ../failed.txt
  fi ) &
  cd ..
done
