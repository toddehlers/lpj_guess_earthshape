...
void FileOutputChannel::finish_row(const Table& table, 
	 ...
	 finish_row(table, lon, lat, year, -1, false, -1, -1);
}
...
void FileOutputChannel::finish_row(const Table& table, 
	 ...
	 finish_row(table, lon, lat, year, day, true, -1, -1);
}
...
void FileOutputChannel::finish_row(const Table& table,
        ...
        finish_row(table, lon, lat, year, day, false, stand, -1);
}
...
void FileOutputChannel::finish_row(const Table& table,
    ...
    finish_row(table, lon, lat, year, day, false, stand, patch);
}
...
void FileOutputChannel::finish_row(const Table& table, 
	 ...
	 if (row.size() < td.columns().size()) {
		  fail("Too few values in a row in table %s\n%d : %d", td.name().c_str(), row.size(), td.columns().size() );
	 }
	 ...
	 if (!printed_header[table.id()]) {
          ...
          // cw stand and patch
          if (stand != -1){
              fprintf(file, "%8s", "Stand");
          }
          if (patch != -1){
              fprintf(file, "%8s", "Patch");
          }
     ...
     // cw stand and patch
     if (stand != -1){
          fprintf(file, "%8d", stand);
     }
     if (patch != -1){
          fprintf(file, "%8d", patch);
     }
...
/// cw new option: annual stand and patch rows
OutputRows::OutputRows(OutputChannel* output_channel,
                           double longitude,
                           double latitude,
                           int year,
...
OutputRows::~OutputRows() {
	 for (size_t i = 0; i < used_tables.size(); i++) {
		  if (used_tables[i]) {
              ...
              if (d==-1){
                  // annual output requested
                  if (st != -1){
                      // per stand & patch or only per stand
                      out->finish_row(Table(i), lon, lat, y, d, st, p);
                  } else {
                      // regular annual output
