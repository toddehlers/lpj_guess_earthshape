...
CommonOutput::CommonOutput() {
	...
	// cw climate
	declare_parameter("file_climate", &file_climate, 300, "Annual climate output file");
...
void CommonOutput::define_output_tables() {
	...
	climate_columns += ColumnDescriptor("co2",               8, 2);
	climate_columns += ColumnDescriptor("ndepo",             9, 3);
	climate_columns += ColumnDescriptor("temp",              8, 2);
	climate_columns += ColumnDescriptor("precip",            8, 2);
	climate_columns += ColumnDescriptor("rad",               8, 2);
	climate_columns += ColumnDescriptor("gdd5",              8, 2);
	...
	// cw climate
	create_output_table(out_climate,        file_climate,        climate_columns);
...
void CommonOutput::outannual(Gridcell& gridcell) {
	...
	double annual_rad(0.0);
	double annual_prec(0.0);
	for (m=0;m<12;m++) {
		annual_temp += gridcell.climate.m_temp[m] / 12;
		annual_rad += gridcell.climate.m_rad[m] / 86400.0 / 12;
		annual_prec += gridcell.climate.m_prec[m];
	}
	outlimit(out,out_climate, gridcell.climate.co2);
	outlimit(out,out_climate, gridcell.climate.andep * m2toha);
	outlimit(out,out_climate, annual_temp);
	outlimit(out,out_climate, annual_prec);
	outlimit(out,out_climate, annual_rad);
	outlimit(out,out_climate, gridcell.climate.agdd5);
	/*

	// Output of tree stand age structure, monthly soil water and 3D vegetation view
	// (Windows shell only - no effect otherwise)

    // cw SubPixel disable windows plot code, causes Malloc errors ?!?!
	if (vegmode==COHORT || vegmode==INDIVIDUAL) {
