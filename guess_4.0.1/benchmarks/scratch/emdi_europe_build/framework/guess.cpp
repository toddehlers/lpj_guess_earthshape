...
// cw SubPixel clone function for climate
void Climate::clone(Gridcell& gridcell, Climate& local_climate) {
    // Serialize this climate to an in-memory stream
    std::stringstream ss;
    ArchiveOutStream aos(ss);
    serialize(aos);
    // ...and deserialize to that climate
    ArchiveInStream ais(ss);
    local_climate.serialize(ais);
}
...
// cw SubPixel
void Landform::serialize(ArchiveStream& arch) {
    arch & id
         & aspect
         & elevation
         & fraction
...
void Site::serialize(ArchiveStream &arch) {
    arch & sand
         & silt
         & clay
         & soc
...
// cw SubPixel alternative stand constructor for landform mode
Stand::Stand(int i, Gridcell* gc, Soiltype& st, landcovertype landcoverX, Landform landformX, int npatch)
  : id(i),
	gridcell(gc),
	soiltype(st),
    ...
    // cw SubPixel delegate init to private helper method for cleaner constructors
    init(npatch, landformX);
}


// cw SubPixel add local_climate to stand constructor
Stand::Stand(int i, Gridcell* gc, Soiltype& st, landcovertype landcoverX, int npatch)
 : id(i),
	...
	Landform default_landform;
	init(npatch, default_landform);
}

void Stand::init(int npatch, Landform landformX) {
...
// cw SubPixel clone climate at stand creation time
void Stand::set_local_climate(){
	gridcell->climate.clone(*gridcell, local_climate);
	local_climate.initdrivers(gridcell->get_lat());
}
...
void Stand::serialize(ArchiveStream& arch) {
	...
	// cw SubPixel
	// TODO: check if landform struct can be serialized like this
	arch & first_year
		& clone_year
		& frac
		& stid
...
const Climate& Stand::get_climate() const {
	...
	// cw SubPixel return stand.local_climate now
	// return get_gridcell().climate;
	return local_climate;
}
...
// cw Subpixel
Site Gridcell::get_site() const {
    return site;
}

void Gridcell::set_site(Site siteX) {
    site = siteX;
}
...
// cw SubPixel - WARNING: NOT TESTED FOR COMPLETENESS
void Gridcell::serialize(ArchiveStream& arch) {
	...
	if (arch.save()) {
		...
		for (unsigned int s = 0; s < number_of_stands; s++) {
            ...
            create_stand(landcover, landform);
...
// cw SubPixel
Stand& Gridcell::create_stand(landcovertype landcover, Landform lf, int no_patch) {
	Stand* stand = new Stand(get_next_id(), this, soiltype, landcover, lf, no_patch);
	double _fraction(lf.fraction);
	if (_fraction > 1){
		_fraction = lf.fraction * 0.01;
	}
	stand->set_gridcell_fraction(_fraction);
	stand->set_local_climate();
	push_back(stand);
