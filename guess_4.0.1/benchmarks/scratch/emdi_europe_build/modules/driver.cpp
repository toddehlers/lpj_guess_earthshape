...
bool pair_compare ( const sortpair& l, const sortpair& r)
{ return l.first < r.first; }

struct soil{double sand, silt, clay;};

double find_distance(soil &self, soil &other) {
	double distance, _sand, _silt, _clay;
	_sand = (self.sand - other.sand);
	_sand = _sand * _sand;
	_silt = (self.silt - other.silt);
	_silt = _silt * _silt;
	_clay = (self.clay - other.clay);
	_clay = _clay * _clay;
	
	distance = _sand + _silt + _clay;
	distance = sqrt(distance);
	return distance;
}


int texture_to_soilcode(double sand, double silt, double clay, double soc){
	int soilcode(0);

	// texture classes according to LPJ code [1-7]
	double data[3][7] = {
		{83., 37., 16.+(2/3.), 60., 50., 27., 46.},  // sand
		{8.,  33., 16.+(2/3.), 20., 12., 25., 19.},  // silt
		{9.,  30., 66.+(2/3.), 20., 38., 48., 35.}}; // clay
	...
	std::vector<sortpair>_tuples;

	// loop over soil codes [1-7]
	for(int i=1; i < 8; ++i){
		soil table_soil;
		table_soil.sand = data[0][i-1];
		table_soil.silt = data[1][i-1];
		table_soil.clay = data[2][i-1];
		
		double d = find_distance(site_soil, table_soil);
		_tuples.push_back(sortpair( d , i));
	}
	
	// sort and return the code of the closest match (texture based only)
	std::sort(_tuples.begin(), _tuples.end(), pair_compare);
	soilcode = _tuples.at(0).second;
	
	// check for vertisols
	if (clay >= 80.) soilcode = 9;
	// check for organic soils
	if (soc >= 30.) soilcode = 8;
	
	return soilcode;
}


// cw SubPixel alternative soilparameters function
void soilparameters(Soiltype& soiltype, int& soilcode, Site site) {
	...
	// scale percent values (0...100) to fractions
	soiltype.sand_frac = site.sand * 0.01;
	soiltype.silt_frac = site.silt * 0.01;
	soiltype.clay_frac = site.clay * 0.01;
	
	// calculate soilcode based on texture and soc
	soilcode = texture_to_soilcode(site.sand, site.silt, site.clay, site.soc);
 
	dprintf("\n Calculated soilcode: %d", soilcode);
	dprintf(" [sand: %g, clay: %g]\n", soiltype.sand_frac, soiltype.clay_frac);
...
// cw SubPixel new argument climate
/// Called each simulation day before any other driver or process functions
void dailyaccounting_gridcell(Gridcell& gridcell, Climate& climate) {
	...
	// cw SubPixel disable gridcell.climate reference, use passed climate
	...
	if (date.day == 0) {
		...
		if (date.year == 0) {
			...
			pftlist.firstobj();
			while (pftlist.isobj) {
				// cw SubPixel wtot NOT ADJUSTED FOR TRUE DEPTH !!!
		...
		// cw SubPixel
		// only execute the following block when climate arg passed is the gridcell.climate instance
		// (and not local_climate), we compare the memory address
		
		if (&gridcell.climate == &climate){
	...
	// f_js_20160503+ monthly climate data for output
	//if (&gridcell.climate != &climate){
	if (date.dayofmonth==0) {
			climate.m_temp[date.month]=0.0;
			climate.m_prec[date.month]=0.0;
			climate.m_gdd5[date.month]=0.0;
			climate.m_rad[date.month]=0.0;
			climate.m_ndepo[date.month]=0.0;
	}
	climate.m_temp[date.month] += climate.temp / date.ndaymonth[date.month];
	climate.m_rad[date.month] += climate.insol / date.ndaymonth[date.month];    // was rad
	climate.m_prec[date.month] += climate.prec;
	climate.m_gdd5[date.month] += max(0.0, climate.temp - 5.0);
	climate.m_ndepo[date.month] += climate.dndep;
	// f_js_20160503-
	...
	climate.drad_31.add(climate.rad);
	climate.mrad = climate.drad_31.mean();
...
void dailyaccounting_patch(Patch& patch) {
    ...
    soil.dwcont_deep[date.day] = soil.wcont_deep;
	...
	if (date.islastday) {
        ...
        soil.mwcont_deep[date.month] = mean(soil.dwcont_deep + date.day - date.ndaymonth[date.month] + 1,
                                            date.ndaymonth[date.month]);
