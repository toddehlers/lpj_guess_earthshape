...
void GridcellOrderedVariable::find_time_dimension() {
	...
	// cw SubPixel extract paleo offset data from calendar attribute
	if (calendar == PALEO) {
		get_paleo_offset_years(calendar_attribute.c_str(), calendar, paleo_refyear);
	} else {
		paleo_refyear = 0;
	}
...
GridcellOrderedVariableNoTime::
GridcellOrderedVariableNoTime(const char* filename,
                        const char* variable) {
    ...
    if (!reduced) {
        if (x_dimension_index == y_dimension_index || ncid_x_dimension == ncid_y_dimension) {
            throw CFError(variable, "Ambiguous x/y dimensions");
        }
    }
  ...
  void GridcellOrderedVariableNoTime::get_lgt_elevation_step(int& lgt_elevation_step) const {
    int status;
    /* Not needed as long as no error checking is included
       nc_type t_type; // NC_TYPE
       size_t t_len;   // vector length of the attribute
       status = nc_inq_att(ncid_file, NC_GLOBAL, "lgt.elevation_step", &t_type, &t_len);
       //std::cout << "status " << status << "\n";
    */
    status = nc_get_att_int(ncid_file, NC_GLOBAL, "lgt.elevation_step", &lgt_elevation_step);
    //std::cout << "status " << status << "\n";
  }
  void GridcellOrderedVariableNoTime::get_lgt_classification(std::string& lgt_classification) const {
    int status;
    size_t t_len;
    char* classi;
    status = nc_inq_attlen(ncid_file, NC_GLOBAL, "lgt.classification", &t_len);
    //std::cout << "status " << status << "\n";
    classi = (char *) malloc(t_len + 1);
    status = nc_get_att_text(ncid_file, NC_GLOBAL, "lgt.classification", classi);
    //std::cout << "status " << status << "\n";
    lgt_classification.assign(classi, t_len);
  }

  // f_js_20171018
  void GridcellOrderedVariableNoTime::get_landforms(std::vector<int>& lf_ids) const {
    char name[NC_MAX_NAME+1];
    int ncid_extra_dimension_var;
    size_t nbr_extra_dimension;
    int status;
    // TODO check the return value in 'status' and throw errors!

    // get the name of the extra dimension
    status = nc_inq_dimname(ncid_file, ncid_extra_dimension, name);
    // get the dimension size
    status = nc_inq_dimlen(ncid_file, ncid_extra_dimension, &nbr_extra_dimension);
    // get the dimension variable id
    status = nc_inq_varid(ncid_file, name, &ncid_extra_dimension_var);
    lf_ids.resize(nbr_extra_dimension);
    status = nc_get_var_int(ncid_file, ncid_extra_dimension_var, &lf_ids.front());
  }

void GridcellOrderedVariableNoTime::find_spatial_dimensions() {
...
void GridcellOrderedVariableNoTime::find_extra_dimension() {
    ncid_extra_dimension = -1;
    extra_dimension_size = 1;
...
bool GridcellOrderedVariableNoTime::load_data_for(size_t x, size_t y) {
    data.resize(extra_dimension_size);
    ...
    // cw SubPixel - do not slip for nodata items
...
bool GridcellOrderedVariableNoTime::load_data_for(size_t landid) {
    
    if (!location_exists(landid)) {
        std::cout << " landid does not exist!!! " << landid << std::endl;
        return false;
    }
    ...
    if (ncid_extra_dimension != -1) {
        ...
        double _lon;
        double _lat;
        get_coords_for(landid, _lon, _lat);
        
        data.resize(1, 0.0);
        const size_t index[] = { landid };
        
        int status = nc_get_var1_double(ncid_file, ncid_var, index, &data.front());
...
bool GridcellOrderedVariableNoTime::is_reduced() const {
    return reduced;
}

GridcellOrderedVariableNoTime::~GridcellOrderedVariableNoTime() {
    close_ncdf(ncid_file);
}


void GridcellOrderedVariableNoTime::get_coords_for(size_t x, size_t y,
                                             double& lon, double& lat) const {
...
void GridcellOrderedVariableNoTime::get_coords_for(size_t landid,
                                             double& lon, double& lat) const {
    if (!location_exists(landid)) {
        std::ostringstream os;
        os << "Attempted to get coordinates for invalid location: " << landid;
...
double GridcellOrderedVariableNoTime::get_value() const {
    return data[0];
}

void GridcellOrderedVariableNoTime::get_values(std::vector<double>& values) const {
    if (values.size() < extra_dimension_size) {
        values.resize(extra_dimension_size);
    }
...
// cw SubPixel
void GridcellOrderedVariableNoTime::get_values(std::vector<double>& values, std::vector<bool>& valid) const {
    if (values.size() < extra_dimension_size) {
        values.resize(extra_dimension_size);
    }
    valid.resize(values.size(), false);
    
    double missing_value(-9999);
    
    std::vector<double>::const_iterator it = data.begin();
    std::vector<bool>::const_iterator mIt = valid.begin();
    int _p(0);
    
    for(; it != data.end() && mIt != valid.end(); ++it, ++mIt)
    {
        if (*it != missing_value){
            valid[_p] = true;
        }
    ...
    // TODO check - there's still something odd here
    for (size_t i = 0; i < extra_dimension_size; ++i) {
        values[i] = data[i];
    }
}
...
bool GridcellOrderedVariableNoTime::location_exists(size_t x, size_t y) const {
...
bool GridcellOrderedVariableNoTime::location_exists(size_t landid) const {
    if (!reduced) {
        throw CFError(variable_name, "Attempted to identify a location with a single index"\
                      " in a file with (x,y)-coordinate system");
    }
...
std::string GridcellOrderedVariableNoTime::get_units() const {
    std::string result;
    if (get_attribute(ncid_file, ncid_var, "units", result)) {
        return result;
    }
...
std::string GridcellOrderedVariableNoTime::get_variable_name() const {
    return variable_name;
}

std::string GridcellOrderedVariableNoTime::get_standard_name() const {
    std::string attribute;
    if (get_attribute(ncid_file, ncid_var, "standard_name", attribute)) {
...
std::string GridcellOrderedVariableNoTime::get_long_name() const {
    std::string result;
    if (get_attribute(ncid_file, ncid_var, "long_name", result)) {
        return result;
    }
...
void GridcellOrderedVariableNoTime::
get_extra_dimension(std::vector<double>& coordinate_values) const {
    if (ncid_extra_dimension == -1) {
        coordinate_values.resize(0);
    }
...
bool GridcellOrderedVariableNoTime::same_spatial_coordinates(int ncid_my_coordvar,
                                                       const GridcellOrderedVariableNoTime& other,
                                                       int ncid_other_coordvar) const {
...
bool GridcellOrderedVariableNoTime::same_spatial_domain(const GridcellOrderedVariableNoTime& other) const {
    if (reduced != other.reduced) {
        return false;
    }
...
void GridcellOrderedVariableNoTime::unpack_data() {

    // SubPixel now skip missing data
    std::vector<bool> is_missing(false, data.size());
    double missing_value;
    
    if (get_attribute(ncid_file, ncid_var, "missing_value", missing_value)){
        for (size_t i = 0; i < data.size(); ++i) {
            is_missing.push_back( (data[i] == missing_value) ? true : false );
        }
    }
