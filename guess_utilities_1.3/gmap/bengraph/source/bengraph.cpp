...
double density_logarithmic(double value,bool ifdual,bool ifzeroclass) {
		...
		if (value<0.0) {
			if (datascale_curr.bounds_neg) {
				if (nclass_neg>1 && pn) {
					...
					if (nclass_neg<5) result*=nclass_neg*0.19999;
				}
				else
					result=0.00001;
			...
			if (datascale_curr.bounds) {
				if (nclass && pp) {
					...
					if (nclass<5) result*=nclass*0.19999;
				}
				else result=0.00001;
				return result;
			}
	...
	if (value>0.0) {
		if (datascale_curr.bounds) {
			pp+=nclass_neg;
			if (nclass_sum && pp) {
				result=pp/(double)(nclass_sum-1);
				if (nclass_sum<5) result*=nclass_sum*0.19999;
				return result;
			}
			else
				return 0.00001;
		...
		if (!datascale_curr.dual) {
			if (datascale_curr.zeroclass && val==0.0) density=0.0;
				...
				if (val>=datascale_curr.minval) {	
					...
					if (nclass<5) density*=nclass*0.19999;
				}
...
int fillint(int id1,int id2,int toaxis,
	...
	else vareabetween(nitem1,data1,nitem2,data2,pattern_style);
